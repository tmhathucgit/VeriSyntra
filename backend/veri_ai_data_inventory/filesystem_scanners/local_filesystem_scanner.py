"""
LocalFilesystemScanner for local directory scanning
...
class LocalFilesystemScanner:
    """Local filesystem scanner with Vietnamese filename support and dynamic configuration"""
    
    def __init__(self, root_path: str):
        """
        Initialize local filesystem scanner with dynamic configuration
    ...
    def discover_files(
        ...
        """
        Discover files with Vietnamese filename support
        ...
        Returns:
            {
                'files': [
                    {
                        'path': str,  # Full path (UTF-8 Vietnamese filenames)
                        'name': str,  # Filename
                        'size': int,
                        'created': datetime,
                        'modified': datetime,
    ...
    def read_sample_content(
        ...
        """
        Read first N bytes of file for content sampling
        ...
        try:
            ...
            logger.info(
                f"[OK] Read {len(content)} bytes from {path.name} "
    ...
    def get_file_metadata(self, file_path: str) -> Dict[str, Any]:
        """
        Get file metadata with Vietnamese filename validation
    ...
    def close(self):
        """Close filesystem scanner and cleanup resources"""
        # No persistent connections for local filesystem
        logger.info("[OK] Filesystem scanner closed")
